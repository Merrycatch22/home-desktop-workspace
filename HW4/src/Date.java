...
public class Date implements Comparable<Date>{
	...
	/**
	 * compares this to the param. Returns positive if this is later, negative if earlier, and 0 if same.
	 */
	@Override
	public int compareTo(Date param)
	{
		int result=year-param.getYear();
		if(result==0){
			result=month-param.getMonth();
			if(result==0){
				result=day-param.getDay();
			}
		}
		return result;
	}
